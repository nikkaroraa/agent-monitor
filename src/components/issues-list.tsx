...
export function IssuesList({ 
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    if (currentView === "backlog" && task.status !== "backlog" && task.status !== "todo") return false;
  ...
  useEffect(() => {
    setFocusIndex(0);
  }, [statusFilter, selectedAgent, currentView]);
      ...
      <div className="flex items-center gap-1 px-4 py-2 border-b border-[--border] bg-[--bg-secondary]">
        {STATUS_TABS.map((tab) => {
          const count = tab.id === "all" 
            ...
            : tasks.filter(t => t.status === tab.id && (!selectedAgent || t.assignee === selectedAgent)).length;
